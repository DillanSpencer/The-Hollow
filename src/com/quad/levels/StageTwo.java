...
public class StageTwo extends State{
	...
	//Light 
		...
		private void setEnemies(){
			...
			g.setPosition(580, 440);
			enemies.add(g);
			
			g = new Golem(tileMap, player, items);
			g.setPosition(800, 440);
			enemies.add(g);
			
			g = new Golem(tileMap, player, items);
			g.setPosition(530,270);
			enemies.add(g);
			
			DarkBat b = new DarkBat(tileMap, player, enemies, items);
			b.setPosition(673, 231);
			enemies.add(b);
		...
		private void setItems(){
			...
			i = new Heart(tileMap, player);
			i.setPosition(157, 268);
			items.add(i);
		...
		private void setWorldBounds(GameContainer gc){
			...
			if(player.getx() >= tileMap.getWidth()){
				gc.getGame().setState(gc, AbstractGame.STAGETHREE);
			}
		}
